...
group.test_new = function(cg)
    ...
    t.assert_equals(result._host, cg.host)
    t.assert_equals(result._port, cg.port)
    t.assert_equals(result._timeout, cg.timeout)
    t.assert_equals(result._default_msg_len, 32768)
end

group.test_connect = function(cg)
    local sock = TCPTransport.new(cg.host, cg.port, cg.timeout)
    local result = sock:connect()
    t.assert(result.success)
    t.assert_not(result.error)
    t.assert_type(result.data, 'boolean')
end

group.test_connect_error = function(cg)
    local sock = TCPTransport.new('168.168.158.1', cg.port, 0.2)
    local result = sock:connect()
...
group.test_write = function(cg)
    ...
    local result = con:connect()
    t.assert(result.success)
    local write = con:write(payload .. cg.delimiter)
    t.assert(write.success)
    t.assert_equals(#(payload .. cg.delimiter), write.data)
    t.assert_equals('Request: ' .. payload .. cg.delimiter, con:read().data)
